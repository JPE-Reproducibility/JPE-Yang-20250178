...
capture log close
log using "$LOG_DIR/table7.log", replace text
...
eststo clear
eststo: reghdfe failure_norm_tot p2013_CL_suit_z if dealcount_norm_mean_00_12!=., absorb(hq#year hq#subseg subseg#year) vce(cluster hq)
quietly estadd ysumm, mean sd replace
eststo: reghdfe ipo_acquired_norm p2013_CL_suit_z if dealcount_norm_mean_00_12!=., absorb(hq#year hq#subseg subseg#year) vce(cluster hq)
quietly estadd ysumm, mean sd replace
eststo: reghdfe other_norm_tot p2013_CL_suit_z if dealcount_norm_mean_00_12!=., absorb(hq#year hq#subseg subseg#year) vce(cluster hq)
quietly estadd ysumm, mean sd replace

esttab est* using "$INTERMEDIATE_DIR/table7_panelA.csv", se replace csv order(p2013_CL_suit_z) ///
...
eststo clear
eststo: reghdfe failure_norm_tot p2013_CL_suit_levelgdp_z p2013_CL_suit_res_levelgdp_z if dealcount_norm_mean_00_12!=., absorb(hq#year hq#subseg subseg#year) vce(cluster hq)
quietly estadd local fe_sc "Yes"
quietly estadd local fe_cy "Yes"
quietly estadd local fe_sy "Yes"
quietly estadd ysumm, mean sd replace
eststo: reghdfe ipo_acquired_norm p2013_CL_suit_levelgdp_z p2013_CL_suit_res_levelgdp_z if dealcount_norm_mean_00_12!=., absorb(hq#year hq#subseg subseg#year) vce(cluster hq)
quietly estadd local fe_sc "Yes"
quietly estadd local fe_cy "Yes"
quietly estadd local fe_sy "Yes"
quietly estadd ysumm, mean sd replace
eststo: reghdfe other_norm_tot p2013_CL_suit_levelgdp_z p2013_CL_suit_res_levelgdp_z if dealcount_norm_mean_00_12!=., absorb(hq#year hq#subseg subseg#year) vce(cluster hq)
quietly estadd local fe_sc "Yes"
quietly estadd local fe_cy "Yes"
quietly estadd local fe_sy "Yes"
quietly estadd ysumm, mean sd replace

esttab est* using "$INTERMEDIATE_DIR/table7_panelB.csv", se replace csv ///
    order(p2013_CL_suit_levelgdp_z p2013_CL_suit_res_levelgdp_z) ///
    star(* 0.10 ** 0.05 *** 0.01) se(%10.3f) b(%10.3f) nobase nocons ///
    nomtitle nonumbers ///
    s(N ymean ysd, label("Number of Obs" "Mean of Dep. Var" "SD of Dep. Var") fmt(0 3 3)) ///
...
tempname fh
file open `fh' using "$TABLE_DIR/table7.csv", write replace text
file write `fh' `","Outcome is (normalized) number of deals for companies that end up",,"' _n
file write `fh' `","(1)","(2)","(3)""' _n
file write `fh' `","Failing","Acquired or IPO","Neither (yet)""' _n
file write `fh' `""Panel A: Baseline Appropriateness Results",,,"' _n

tempname pa
file open `pa' using "$INTERMEDIATE_DIR/table7_panelA.csv", read text
file read `pa' line
while r(eof)==0 {
...
file write `fh' `""Panel B: Vertical versus Horizontal Results",,,"' _n

file open `pa' using "$INTERMEDIATE_DIR/table7_panelB.csv", read text
file read `pa' line
while r(eof)==0 {
    file write `fh' `"`line'"' _n
    file read `pa' line
...
file write `fh' `""Sector x Year FE","Yes","Yes","Yes""' _n
file close `fh'

erase "$INTERMEDIATE_DIR/table7_panelA.csv"
erase "$INTERMEDIATE_DIR/table7_panelB.csv"
